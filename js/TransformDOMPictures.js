function App(target, domDiv, svgTool, point){
	
	this.dom = domDiv;
	var toolElem = svgTool;
	this.tool = new DOMTransformTool(toolElem);
	
	this.displayList = [
		// new Picture(document.querySelector('.image'),0,0),
		new Picture(target, point.x, point.y),
	];
	...
	this.tool.setTarget(this.displayList[0].transform);
	
	// draws initial screen
	this.render();
}

App.create = function(target, domDiv, svgTool, point) {
	App.instance = new App(target, domDiv, svgTool, point);
	return App.instance;
}
App.instance = null;
...
App.prototype.getCustomControls = function(){
	...
	return [
		...
		new DOMControl(Control.ROTATE_SCALE, 1, 0, 0, 0, 18),
		new DOMControl(Control.ROTATE_SCALE, 0, 1, 0, 0, 18),
		new DOMControl(Control.ROTATE_SCALE, 0, 0, 1, 0, 18),
		new DOMControl(Control.ROTATE_SCALE, 1, 1, 1, 1, 18),
...
App.prototype.down = function(event){
	...
	if (controlled){
		...
		document.querySelector('body').addEventListener(Mouse.MOVE, this.move);
